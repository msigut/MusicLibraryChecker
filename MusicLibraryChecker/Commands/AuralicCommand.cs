...
namespace MusicLibraryChecker.Commands
{
    internal class AuralicCommand
    {
		...
		/// </summary>
		public static string[] MUSIC_EXTS = new[] { ".flac", ".mp3" };
        ...
        public static void Execute(IReadOnlyCollection<string> paths, bool recursive, bool fix, int minWidth = 500, int minHeight = 500, bool bookletWarning = false)
        {
			foreach (var path in paths)
			{
				...
				var dirs = DirectoriesContainsExtensions(pathInfo, MUSIC_EXTS, searchOption: (recursive ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly));
				foreach (var dir in dirs)
				{
					...
					foreach (var hidden in hiddens.Where(x => x.Length <= (5 * 1024)))
					{
		...
		/// </summary>
		private static IReadOnlyCollection<DirectoryInfo> DirectoriesContainsExtensions(DirectoryInfo directory, string[] findExtensions, SearchOption searchOption = SearchOption.TopDirectoryOnly)
		{
			var dirs = directory.GetDirectories("*", searchOption);

			return dirs.Where(x => x.GetFiles().Any(y => findExtensions.Contains(y.Extension.ToLowerInvariant())))
